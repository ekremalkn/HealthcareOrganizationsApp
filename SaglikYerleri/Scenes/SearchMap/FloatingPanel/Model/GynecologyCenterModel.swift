...
struct GynecologyCenter: Codable, OrganizationModel, SharedCell2DataProtocol {
    let name, city, country, street: String?
    let streetv2, phone, fax, website: String?
    let latitude, longitude: Double?

    var sharedCell2ImageBackgroundColor: UIColor {
        return MainHorizontalCollectionData.categoryType(.gynecologyCenters).tintAndBackgroundColor
    }
    
    var sharedCell2Image: UIImage {
        return MainHorizontalCollectionData.categoryType(.gynecologyCenters).image
    }
