    ...
    private func buttonCallbacks() {
        ...
        selectionPopUpView.openMapButton.rx.tap.subscribe { [weak self] _ in
            ...
            DispatchQueue.main.async { [weak self] in
                guard let self else { return }
                let elementToCheck = selectedCategory.selectedCategoryType
                let isSelectedCategoryFree = viewModel.freeCollectionCategories.contains { $0.selectedCategoryType == elementToCheck }
                ...
                if isSelectedCategoryFree {
                    ...
                    if !isUserSubscribe {
                        selectionPopUpCoordinator?.openPayWall(vc: self)
